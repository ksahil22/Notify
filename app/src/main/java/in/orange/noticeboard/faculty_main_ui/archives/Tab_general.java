...
public class Tab_general extends Fragment implements CustomRecyclerAdapter.OnNoticeClick{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceStste) {
        ...
        l=NoticeDatabase.getNoticeDatabaseInstance(getContext()).getAllNotices("General", -1,"1");
        if(l==null)
        {
        ...
        pullToRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                try {
                    List<DisplayNotice> temp = NoticeDatabase.getNoticeDatabaseInstance(getContext()).getAllNotices("General", Integer.parseInt(l.get(0).id),"1");
                    l.addAll(0,temp);
                }catch(Exception e){
                    try{
                        l.addAll(NoticeDatabase.getNoticeDatabaseInstance(getContext()).getAllNotices("General", -1,"1"));
                    }catch(Exception ee){}
                }
                myadapter.notifyDataSetChanged();
